...
class CDN_Integration_Admin {
    ...
    /**
     * The GitHub OAuth instance.
     *
     * @since 2.0.0
     * @access protected
     * @var CDN_Integration_GitHub_OAuth $github_oauth
     */
    protected $github_oauth;
    ...
    public function enqueue_admin_assets() {
        ...
        // Add GitHub setup styles
        wp_enqueue_style(
            'wp-cdn-integration-github-setup',
            WP_CDN_INTEGRATION_PLUGIN_URL . 'admin/css/github-setup.css',
            array('wp-cdn-integration-admin'),
            WP_CDN_INTEGRATION_VERSION
        );
        ...
        wp_localize_script('wp-cdn-integration-admin', 'wpCdnAdmin', array(
            ...
            'i18n' => array(
                ...
                'connecting' => __('Connecting...', 'wp-cdn-integration'),
                'connectWithGitHub' => __('Connect with GitHub', 'wp-cdn-integration'),
                'connectionError' => __('Connection error occurred', 'wp-cdn-integration'),
    ...
    public function render_main_page() {
        ...
        <div class="wrap cdn-integration-dashboard">
            ...
            if (isset($_GET['github_success']) && $_GET['github_success'] == '1') {
                echo '<div class="notice notice-success is-dismissible"><p><strong>' . __('Success!', 'wp-cdn-integration') . '</strong> ' . __('GitHub connected successfully! Your repository is now ready to use.', 'wp-cdn-integration') . '</p></div>';
            }
            if (isset($_GET['github_error']) && !empty($_GET['github_error'])) {
                $error_message = sanitize_text_field($_GET['github_error']);
                echo '<div class="notice notice-error is-dismissible"><p><strong>' . __('GitHub Connection Error:', 'wp-cdn-integration') . '</strong> ' . esc_html($error_message) . '</p></div>';
            }
            ?>
...
?>
                <div class="cdn-tab-content">
                    ...
                    <div class="cdn-tab-pane" id="github-tab">
                        <div class="dashboard-card">
                            <div class="card-header">
                                <h2><?php esc_html_e('GitHub Integration', 'wp-cdn-integration'); ?></h2>
                            </div>
                            <div class="card-content">
                                <?php
                                $options = get_option('wp_cdn_integration_settings');
                                ...
                                $github_configured = (!empty($username) && !empty($repository) && !empty($token));
                                ?>
                                
                                <?php if (!$github_configured): ?>
                                <!-- Quick Auto Setup Section -->
                                <div class="cdn-auto-setup-section">
                                    <div class="cdn-setup-card auto-setup-card">
                                        <div class="setup-header">
                                            <span class="setup-icon">🚀</span>
                                            <h3><?php esc_html_e('Quick Setup (Recommended)', 'wp-cdn-integration'); ?></h3>
                                        </div>
                                        <p class="setup-description"><?php esc_html_e('Connect your GitHub account automatically in just one click. We\'ll handle all the technical setup for you.', 'wp-cdn-integration'); ?></p>
                                        
                                        <div class="auto-connect-wrapper">
                                            <button type="button" id="auto-connect-github" class="button button-primary auto-connect-btn">
                                                <span class="dashicons dashicons-admin-links"></span>
                                                <?php esc_html_e('Auto Connect with GitHub', 'wp-cdn-integration'); ?>
                                            </button>
                                            <p class="connect-note"><?php esc_html_e('This will automatically create a repository and configure all settings for you.', 'wp-cdn-integration'); ?></p>
                                            
                                            <div class="popup-instructions">
                                                <span class="dashicons dashicons-info"></span>
                                                <?php esc_html_e('A popup window will open for GitHub authorization. Please complete the process in the popup window.', 'wp-cdn-integration'); ?>
                                            </div>
                                            
                                            <div class="auto-connect-status" style="display: none;"></div>
                                        </div>
                                        
                                        <div id="auto-setup-progress" class="cdn-progress-section" style="display: none;">
                                            <div class="cdn-progress-bar">
                                                <div class="cdn-progress-fill" id="auto-setup-progress-fill"></div>
                                            </div>
                                            <p id="auto-setup-status" class="progress-status"><?php esc_html_e('Connecting...', 'wp-cdn-integration'); ?></p>
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="cdn-divider">
                                    <span class="divider-text"><?php esc_html_e('OR', 'wp-cdn-integration'); ?></span>
                                </div>
                                
                                <!-- Manual Setup Section -->
                                <div class="cdn-manual-setup-section">
                                    <div class="cdn-setup-card manual-setup-card">
                                        <div class="setup-header">
                                            <span class="setup-icon">🔧</span>
                                            <h3><?php esc_html_e('Manual Setup', 'wp-cdn-integration'); ?></h3>
                                            <button type="button" class="cdn-toggle-manual">
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </button>
                                        </div>
                                        <p class="setup-description"><?php esc_html_e('Configure GitHub connection manually with your own settings.', 'wp-cdn-integration'); ?></p>
                                        
                                        <div class="cdn-manual-form" style="display: none;">
                                            
                                            <form method="post" action="options.php" class="cdn-settings-form">
                                                <?php settings_fields('wp_cdn_integration_settings'); ?>
                                                
                                                <div class="github-fields-grid">
                                                    <div class="field-group">
                                                        <label for="wp-cdn-github-username"><?php esc_html_e('GitHub Username', 'wp-cdn-integration'); ?></label>
                                                        <input type="text" id="wp-cdn-github-username" name="wp_cdn_integration_settings[github_username]" value="<?php echo esc_attr($username); ?>" class="regular-text" placeholder="your-username">
                                                        <p class="field-description"><?php esc_html_e('Your GitHub username.', 'wp-cdn-integration'); ?></p>
                                                    </div>
                                                    
                                                    <div class="field-group">
                                                        <label for="wp-cdn-github-repository"><?php esc_html_e('Repository Name', 'wp-cdn-integration'); ?></label>
                                                        <input type="text" id="wp-cdn-github-repository" name="wp_cdn_integration_settings[github_repository]" value="<?php echo esc_attr($repository); ?>" class="regular-text" placeholder="my-cdn-repo">
                                                        <p class="field-description"><?php esc_html_e('The name of your GitHub repository.', 'wp-cdn-integration'); ?></p>
                                                    </div>
                                                    
                                                    <div class="field-group">
                                                        <label for="wp-cdn-github-branch"><?php esc_html_e('Branch Name', 'wp-cdn-integration'); ?></label>
                                                        <input type="text" id="wp-cdn-github-branch" name="wp_cdn_integration_settings[github_branch]" value="<?php echo esc_attr($branch); ?>" class="regular-text" placeholder="main">
                                                        <p class="field-description"><?php esc_html_e('The branch to use (default: main).', 'wp-cdn-integration'); ?></p>
                                                    </div>
                                                    
                                                    <div class="field-group full-width">
                                                        <label for="wp-cdn-github-token"><?php esc_html_e('GitHub Personal Access Token', 'wp-cdn-integration'); ?></label>
                                                        <input type="password" id="wp-cdn-github-token" name="wp_cdn_integration_settings[github_token]" value="<?php echo esc_attr($token); ?>" class="regular-text" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx">
                                                        <p class="field-description"><?php esc_html_e('Your GitHub Personal Access Token with repository access. Generate one at GitHub Settings > Developer settings > Personal access tokens.', 'wp-cdn-integration'); ?></p>
                                                    </div>
                                                </div>
                                                
                                                <div class="manual-form-actions">
                                                    <button type="button" id="test-github-connection" class="button button-secondary">
                                                        <span class="dashicons dashicons-cloud"></span>
                                                        <?php esc_html_e('Test Connection', 'wp-cdn-integration'); ?>
                                                    </button>
                                                    <?php submit_button(__('Save GitHub Settings', 'wp-cdn-integration'), 'primary', 'submit', false); ?>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                ...
                                <?php else: ?>
                                <!-- Connected State -->
                                <div class="cdn-connected-section">
                                    <div class="notice notice-success" style="margin-bottom: 20px; padding: 20px; background: #d4edda; border: 1px solid #c3e6cb; border-radius: 8px;">
                                        <h3 style="margin-top: 0; color: #155724;"><?php esc_html_e('✅ GitHub Connected Successfully!', 'wp-cdn-integration'); ?></h3>
                                        <p style="margin-bottom: 0; font-size: 16px;"><?php printf(__('Connected to repository: %s/%s', 'wp-cdn-integration'), '<strong>' . esc_html($username) . '</strong>', '<strong>' . esc_html($repository) . '</strong>'); ?></p>
                                    </div>
                                    
                                    <div class="cdn-connection-actions" style="text-align: center; margin: 20px 0;">
                                        <button type="button" id="test-github-connection" class="button button-secondary" style="margin-right: 10px;">
                                            <span class="dashicons dashicons-cloud"></span>
                                            <?php esc_html_e('Test Connection', 'wp-cdn-integration'); ?>
                                        </button>
                                        <button type="button" id="disconnect-github" class="button button-link-delete">
                                            <span class="dashicons dashicons-dismiss"></span>
                                            <?php esc_html_e('Disconnect', 'wp-cdn-integration'); ?>
                                        </button>
                                    </div>
...
public function sanitize_settings($input) {
    ...
    public function ajax_test_connection() {
        ...
        $connection_result = $this->github_api->test_connection();
        
        if (is_array($connection_result)) {
            if ($connection_result['success']) {
                wp_send_json_success(array(
                    'message' => $connection_result['message'],
                    'repository' => isset($connection_result['repository']) ? $connection_result['repository'] : null
                ));
            } else {
                wp_send_json_error(array(
                    'message' => $connection_result['message'],
                    'code' => isset($connection_result['code']) ? $connection_result['code'] : 'unknown_error'
                ));
            }
        } else {
            // Legacy boolean response
    ...
    /**
     * AJAX handler for auto GitHub connection.
     */
    public function ajax_auto_connect_github() {
        // Check nonce
        if (!check_ajax_referer('wp-cdn-integration-nonce', 'nonce', false)) {
            wp_send_json_error(array('message' => __('Security check failed.', 'wp-cdn-integration')));
        }
        ...
        try {
            // Initialize GitHub OAuth if not already done
            if (!$this->github_oauth) {
                $this->github_oauth = new CDN_Integration_GitHub_OAuth();
            }

            // Start auto connection process
            $result = $this->github_oauth->start_auto_connection();
            ...
            if ($result && isset($result['success']) && $result['success']) {
                ...
                $error_message = isset($result['message']) ? $result['message'] : __('Failed to initialize GitHub connection', 'wp-cdn-integration');
                wp_send_json_error(array(
            ...
            wp_send_json_error(array(
                'message' => sprintf(__('Connection error: %s', 'wp-cdn-integration'), $e->getMessage())
            ));
        }
    }

    /**
     * AJAX handler for disconnecting GitHub.
     */
    public function ajax_disconnect_github() {
        ...
        try {
            ...
            wp_send_json_success(array(
                'message' => __('GitHub disconnected successfully', 'wp-cdn-integration')
            ));
        } catch (Exception $e) {
            wp_send_json_error(array(
                'message' => sprintf(__('Disconnect error: %s', 'wp-cdn-integration'), $e->getMessage())
            ));
        }
    }

    /**
     * AJAX handler for GitHub OAuth callback.
     */
    public function ajax_github_callback() {
        ...
        if ($error) {
            ...
            wp_redirect(admin_url('admin.php?page=wp-cdn-integration&github_error=' . urlencode($error_description)));
            exit;
        }
        
        // Validate required parameters
        if (empty($code) || empty($state)) {
            wp_redirect(admin_url('admin.php?page=wp-cdn-integration&github_error=' . urlencode('Invalid callback parameters')));
            exit;
        }
        ...
        try {
            ...
            // Complete auto connection process
            $result = $this->github_oauth->complete_auto_connection($code, $state);

            if ($result && isset($result['success']) && $result['success']) {
                // Redirect back to admin with success message
                wp_redirect(admin_url('admin.php?page=wp-cdn-integration&github_success=1'));
                exit;
            } else {
                ...
                wp_redirect(admin_url('admin.php?page=wp-cdn-integration&github_error=' . urlencode($error_message)));
                exit;
            }
        } catch (Exception $e) {
            wp_redirect(admin_url('admin.php?page=wp-cdn-integration&github_error=' . urlencode($e->getMessage())));
            exit;
        }
    }
